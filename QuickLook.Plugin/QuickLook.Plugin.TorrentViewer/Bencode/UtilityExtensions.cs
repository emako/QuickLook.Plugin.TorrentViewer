...
namespace QuickLook.Plugin.TorrentViewer.Bencode;
...
internal static class UtilityExtensions
{
    ...
    {
        return new string(chars.Span.ToArray());
    }

    public static int GetBytes(this Encoding encoding, ReadOnlySpan<char> chars, Span<byte> bytes)
    {
        char[] charArray = chars.ToArray();
        byte[] byteArray = encoding.GetBytes(charArray);

        if (byteArray.Length > bytes.Length)
        {
            throw new ArgumentException("The byte span is too small to hold the result.");
        }
